...
void simulationDataBase::createParticlesInFieldsRootNode()
{
    ...
    //! analysis type
    //! --------------
    Property::analysisType analysisType = Property::analysisType_particlesInFields;
    data.setValue(analysisType);
    Property property_analysisType("Analysis type",data,Property::PropertyGroup_StepControls);
    ...
    //! create the tabular data
    //! ------------------------
    nodeParticlesInFieldsAnalysisSettings->createTabularData(vecLoad);

    //! -----------------------------
    //! time tag and parent time tag
    //! -----------------------------
    nodeParticlesInFieldsAnalysisSettings->addTimeTag();
    QString parentTimeTag = ParticlesInFieldsAnalysisItem->data(Qt::UserRole).value<SimulationNodeClass*>()->getPropertyValue<QString>("Time tag");
    data.setValue(parentTimeTag);
    ...
    ParticlesInFieldsAnalysisSettingsItem->setData("Analysis settings", Qt::DisplayRole);
    ParticlesInFieldsAnalysisItem->appendRow(ParticlesInFieldsAnalysisSettingsItem);

    mainTreeTools::addSolution(ParticlesInFieldsAnalysisItem);
    mainTreeTools::addSolutionInformation(ParticlesInFieldsAnalysisItem->child(ParticlesInFieldsAnalysisItem->rowCount()-1,0));
}
