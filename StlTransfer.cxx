...
void StlTransfer::RetrieveMesh (const TopoDS_Shape&  Shape,
                                const occHandle(StlMesh_Mesh)& Mesh)
{
    cout<<"StlTransfer::RetrieveMesh()->____function called____"<<endl;


    //! one single domain - modification GB
    //! without this, @ line 171,172,173 only the points of the FIRST inserted domain are retrieved
    //Mesh->AddDomain();

    for (TopExp_Explorer itf(Shape,TopAbs_FACE); itf.More(); itf.Next())
    {
        cout<<"____retrieving the triangulation from the face____"<<endl;

        TopoDS_Face face = TopoDS::Face(itf.Current());
        TopLoc_Location Loc, loc;
        occHandle(Poly_Triangulation) theTriangulation = BRep_Tool::Triangulation(face, Loc);
        if (theTriangulation.IsNull())
        {
            cout<<"____the current face has not a mesh____"<<endl;
            continue; //Meshing was not done for this face!
        }
        cout<<"____number of stl triangles for the current face: "<<theTriangulation->NbTriangles()<<"____"<<endl;

        Poly_Array1OfTriangle theTriangles(1,theTriangulation->NbTriangles());
        theTriangles.Assign(theTriangulation->Triangles());

        cout<<"____adding domain____"<<endl;
        cout<<"____deflection: "<<theTriangulation->Deflection()<<"____"<<endl;
        ...
        cout<<"____compute the normal to the face____"<<endl;
    ...
    cout<<"StlTransfer::RetrieveMesh()->____stl mesh retrieved____"<<endl;
}
