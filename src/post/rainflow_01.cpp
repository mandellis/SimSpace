...
//! details:
//! ----------------------
rainflow_01::rainflow_01(QObject *parent): QObject(parent)
{
   ;
}
...
//! details:
//! ------------------------
rainflow_01::rainflow_01(GeometryTag loc, QObject *parent): QObject(parent),myLoc(loc)
{
    ;
}
...
//! details:
//! ------------------------
rainflow_01::rainflow_01(GeometryTag loc, fatigueModel fm, QObject *parent): QObject(parent),
 ...
 *-------------------------------------------------------------*/
int rainflow_01::rf3(double *array_ext,	// (in) an array of turning points (see sig2ext on how to get these)
 ...
 * Based on Adam Nieslony's rainflow.c for Matlab.
 *-------------------------------------------------------------*/
int rainflow_01::rf5(double *array_ext, int nr, double *array_t, double *array_out){
 ...
 *-------------------------------------------------------------*/
int rainflow_01::sig2ext(double *sig, double *time_sig, long n, int clsn,
 ...
 * as well.
 *-------------------------------------------------------------*/
double rainflow_01::arr_min(double *sig, int n, int *pos)
{
    int i, ind=0;
 ...
 * as well.
 *-------------------------------------------------------------*/
double rainflow_01::arr_max(double *sig, int n, int *pos){
    int i, ind=0;
 ...
 * needed.
 *-------------------------------------------------------------*/
double *rainflow_01::diff(double *vec, int n)
{
 ...
 *-------------------------------------------------------------*/
int rainflow_01::repl(double *x, int *filt, int n, double *x_repl)
{
    int i, j;
    j = 0;
    for (i=0; i<n; i++){
...
//! details:
//! ------------------
bool rainflow_01::perform(std::map<int, std::vector<double>> strainDistTimeHistory, std::map<int,double> &damageDist)
{
    cout<<"rainflow_01::perform()->____function called____"<<endl;
    for(std::map<int, std::vector<double>>::iterator it = strainDistTimeHistory.begin(); it!= strainDistTimeHistory.end(); it++)
    {
        ...
        long timeSize = timeHistory.size();
        double *tH= NNEW(double, timeSize);
        for (int i=0; i<(timeSize); i++)
        {
            tH[i] = timeHistory[i];
            cout<<timeHistory[i]<<endl;
        }
...
//! details:
//! -----------------------
double rainflow_01::damage_index(double *y, long timeSize)
{
    cout<<"rainflow_01::damage_index()->____function called____"<<endl;
    ...
    int np = this->rf3(y,timeSize,ext);
    cout<<"rainflow_01::damage_index()->____number of data____"<<np<<endl;

    //size_t NbData = B.size();
    for(int i=0; i<np; i++)
    {
        deltaEps = ext[3*i];
        cout<<"rainflow_01::damage_index()->____ext value____"<<ext[3*i]<<"___"<<ext[3*i+1]<<"___"<<ext[3*i+2]<<endl;
...
//! -------------------------------------------------
double rainflow_01::solve(double eps, double epsF, double c, double sigmaF, double E, double b)
{
...
//! details:
//! ----------------------
double rainflow_01::solve_exact(double eps, double epsF, double c, double sigmaF, double E, double b)
{
