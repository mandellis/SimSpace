                                                        ...
                                                        occHandle(MeshVS_DataSource) &mainMesh2D)
{
    ...
    bool isVolume = false;
    userMessage mr = myNetgenMesher->perform(bodyIndex,isVolume,mainMesh2D,dummyDS,vecUnassociatedElements);
    if(mr.isDone)
    {
        myMeshDB->ArrayOfMeshDS2D.insert(bodyIndex,mainMesh2D);
        myMeshDB->ArrayOfMeshDS.insert(bodyIndex,dummyDS);
...
//! details:  internally calls prismaticLayer. The prismatic mesh
//!           is recorded into the mesh database
//! --------------------------------------------------------------
...
{    
    ...
    if(isDone)
    {
        mr.message = QString("The prismatic mesh at boundary has been generated");
        myMeshDB->ArrayOfMeshDS.insert(bodyIndex,prismaticMeshDS);
    }
    else
    {
        mr.message = QString("Error in generating the mesh at boundary");
                                                   ...
                                                   occHandle(MeshVS_DataSource) &mainMesh3D)
{
    ...
    else
    {
        ...
        if(algo ==0)
        {
            ...
            //! using the new globalNodeIDs of the final prismatic + volume mesh.
            //! To this aim:
            ...
            //! renumber nodes and put the mesh data sources into the database
            //! ---------------------------------------------------------------
            int NbGeometryFaces = myMeshDB->MapOfBodyTopologyMap.value(bodyIndex).faceMap.Extent();
            for(int faceNr = 1; faceNr<=NbGeometryFaces; faceNr++)
            {
                cout<<"____renumbering face #"<<faceNr<<"____"<<endl;
                if(myMeshDB->ArrayOfMeshDSOnFaces.getValue(bodyIndex,faceNr).IsNull()) continue;
...
userMessage MesherClass::Tetgen_generateVolumeMesh(int bodyIndex, int preserveSurfaceMesh, bool runInMemory, int done)
{
    ...
    if(!hasPrismaticFaces)  // standard meshing without boundary mesh
    {
        ...
        if(runInMemory==false)
        {
            ...
            {
                mr.isDone = false;
                mr.message = QString("Tetgen failure. Tetgen exit code %1").arg(exitCode);
                cout<<mr.message.toStdString()<<endl;
                return mr;
            }
        }
        ...
        {
            //! ----------------------------------------
            //! mesh the volume using tetgen as library
            //! ----------------------------------------
            ...
            tetgenio meshOut;
            QString message;
            bool isVolDone = myTetMesher->perform(&meshOut, bodyIndex, message);
            if(isVolDone==false)
            {
                mr.isDone = false;
                mr.message = QString("Tetgen meshing failure on body index %1").arg(bodyIndex)+" :"+message;
                cout<<mr.message.toStdString()<<endl;
                return mr;
            }
    ...
    else    // meshing with boundary mesh
    {
        ...
        //!
        //! **********************************************************
        occHandle(Ng_MeshVS_DataSourceFace) theLastInflatedMesh;                // this is the watertight surface input mesh for tetgen
        QList<occHandle(Ng_MeshVS_DataSourceFace)> listOfInflatedMeshes;
        mr = this->PrismaticLayers_generatePrismaticMesh(bodyIndex,theLastInflatedMesh,listOfInflatedMeshes);
        if(mr.isDone==false)
        {
        ...
        //! --------------------------------------------------------------
        //! the volume mesh generated starting from "theLastInflatedMesh"
        //! it can be generated using tetgen.exe or tet.lib
        //! --------------------------------------------------------------
        occHandle(Ng_MeshVS_DataSource3D) tetgenVolumeMeshDS;
        ...
        if(runInMemory==false)  // tetgen run on disk
        {
            ...
            //! --------------------------------------------------------------------------
            //! this will generate <bodyName>1.ele, <bodyName>1.face, <bodyName>1.edge
            //! the file name is internally retrieved by tetMesher from the mesh database
            //! --------------------------------------------------------------------------
        ...
        else
        {
            ...
            tetgenio meshOut;
            QString message;
            bool isDone = myTetMesher->perform(&meshOut, bodyIndex, message);
            if(isDone==false)
            {
                mr.isDone = false;
                mr.message = QString("Tetgen meshing failure on body index %1").arg(bodyIndex)+" :"+message;
        ...
        //! retrieve the prismatic 3D mesh recorded into the database by "PrismaticLayers_generatePrismaticMesh"
        //! -----------------------------------------------------------------------------------------------------
        occHandle(Ng_MeshVS_DataSource3D) prismatic3DMesh =
                occHandle(Ng_MeshVS_DataSource3D)::DownCast(myMeshDB->ArrayOfMeshDS.value(bodyIndex));
        ...
        //! ---------------------------------------------------------------
        //! insert the boundary + volume mesh into the mesh database
        //! create and insert also the surface mesh, created topologically
        //! ---------------------------------------------------------------
        if(mainMesh3D->myFaceToElements.isEmpty()) mainMesh3D->buildFaceToElementConnectivity();
        occHandle(Ng_MeshVS_DataSource2D) surfaceMesh = new Ng_MeshVS_DataSource2D(mainMesh3D);
        myMeshDB->ArrayOfMeshDS.insert(bodyIndex,mainMesh3D);
        ...
        {
            //! ------------------------------------------------
            //! renumber the nodes of the face mesh datasources
            //! put the renumbered meshes into the database
            //! ------------------------------------------------
                                                       ...
                                                       occHandle(MeshVS_DataSource) &mainMesh3D)
{
    ...
    if(!isInflationDefinedOnBody)
    {
        ...
        std::vector<std::vector<double>> vecUnassociatedElements;
        bool isVolume = true;
        mr = myNetgenMesher->perform(bodyIndex,isVolume,mainMesh2D,mainMesh3D,vecUnassociatedElements);
    ...
    else
    {
        ...
        else
        {
            ...
            std::vector<std::vector<double>> vecUnassociatedElements;
            bool isVolume = true;
            mr = myNetgenMesher->perform(bodyIndex,isVolume,mainMesh2D,mainMesh3D,vecUnassociatedElements); // perform on an STL input
...
//! details:
//! ----------------------
bool MesherClass::processMesh(int bodyIndex)
{
    cout<<"MesherClass::processMesh()->____function called____"<<endl;
    std::string fileName("D:/STL1.stl");
    std::map<int,occHandle(Ng_MeshVS_DataSourceFace)> mapOfFaceDS;
    ...
    for(int faceNr = 1; faceNr<=NbGeometryFaces; faceNr++)
    {
        ...
        if(aMeshDS.IsNull())
        {
            cout<<"MesherClass::processMesh()->____face #"<<faceNr<<" is null____"<<endl;
            continue;
        }
        occHandle(Ng_MeshVS_DataSourceFace) &aFaceMeshDS = occHandle(Ng_MeshVS_DataSourceFace)::DownCast(aMeshDS);
        mapOfFaceDS.insert(std::make_pair(faceNr,aFaceMeshDS));
    }
    MeshTools::faceDataSourcesToExtendedStlFile(mapOfFaceDS,fileName);
    STLdoctor aSTLDoctor;
    //aSTLDoctor.simplifyMesh(QString("D:/STL1.stl"),QString("D:/STL2.stl"),1,-1,2);
    aSTLDoctor.performMeshFix(QString("D:/STL1.stl"),QString("D:/STL3.stl"));
    ...
    QString surfaceMeshFilePath("D:/Stl3.stl");
    ...
    //! -----------------------------------
    occHandle(Ng_MeshVS_DataSource2D) surfaceMeshDS = new Ng_MeshVS_DataSource2D(overallSTL);
    myMeshDB->ArrayOfMeshDS2D.insert(bodyIndex,surfaceMeshDS);
