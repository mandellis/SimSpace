...
//
//****************************************************************************
bool diagonalie (int im1, int ip1, int n, int next_node[], double x[], double y[])
{
    int first;
    int j;
    int jp1;
...
//! -------------------------
bool getTriangles(int m, int n, int a[], QList<QList<int>> &triangles)
{
    if(getTriangles_some(m, n, a, 1, 1, m, n, triangles))
    {
        return true;
    }
    else
...
//! ----------------------------
//! function: getTriangles_some
//! ----------------------------
bool getTriangles_some(int m, int n, int a[], int ilo, int jlo, int ihi, int jhi, QList<QList<int>> & triangles)
{
# define INCX 10
...
//
//****************************************************************************
bool in_cone (int im1, int ip1, int n, int prev_node[], int next_node[], double x[], double y[] )
{
    int i;
    int im2;
    double t1;
...
//
//****************************************************************************80
bool polygon_triangulate (int n, double x[], double y[], int* triangles)
{
    double angle;
    double area;
    bool *ear;
    ...
    for(node2 = 0; node2 < n; node2++)
    {
        ...
        if (fabs(angle)<=1.0)
        {
            //cerr << "POLYGON_TRIANGULATE - error\n";
            //cerr << "  Polygon has an angle smaller than 1.0 degrees. ";
    ...
    if (area < 0.0)
    {
        //cerr << "\n";
        //cerr << "POLYGON_TRIANGULATE - Fatal error!\n";
        cerr << "  Polygon has zero or negative area. Area = "<<area<<endl;
        //reverse_array(x,n);
        //reverse_array(y,n);
        //area = polygon_area (n, x, y);
        //cerr << "  Now reversing order. Area = "<<area<<endl;
        return false;
    }
    */
