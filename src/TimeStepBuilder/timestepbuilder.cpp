...
//! details:
//! ----------------------
TimeStepBuilder::TimeStepBuilder(const QString &inputFile, const QString &outputDir, QWidget *parent):
    QDialog(parent),
...
{
    cout<<"TimeStepBuilder::~TimeStepBuilder()->____DESTRUCTOR CALLED____"<<endl;
}
...
bool TimeStepBuilder::openFile()
{
    ...
    if(!QFile::exists(myInputFile))
    {
        QString selectedFilter;
        timeHistoryFile = QFileDialog::getOpenFileName(this,"Select time history file", myOutputDir,"*.txt",&selectedFilter);
    ...
    {
        timeHistoryFile = myInputFile;
    }

    FILE *timeHistoryFilePointer = fopen(timeHistoryFile.toStdString().c_str(),"r");
    if(timeHistoryFilePointer==NULL) return false;
...
void TimeStepBuilder::acceptTimeDivision()
{
    if(!myPlot->getTimeStepMarkerList().isEmpty())
    {
        ...
        //! ---------------------------
        QString timeStepFile = myOutputDir+"/timepoints.out";
